...
// ==== Challenge 1: Use .forEach() ==== X
// The event director needs both the first and last names of each runner for their running bibs.  Combine both the first and last names into a new array called fullName.
let fullName = [];

runners.forEach(function(runnerObject) {
  fullName.push(`${runnerObject.first_name} ${runnerObject.last_name}`)
});

console.log(fullName);

// ==== Challenge 2: Use .map() ==== X
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = runners.map(runnerObject => runnerObject.first_name.toUpperCase());

console.log(allCaps);

// ==== Challenge 3: Use .filter() ==== X
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];

runners.filter(function(runnerObject) {
  if(runnerObject.shirt_size === 'L') {
    largeShirts.push(`${runnerObject.first_name} ${runnerObject.last_name}`)
  }
});

console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ==== X
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = runners.reduce(function(total,runnerObject) {
  const sum = total + runnerObject.donation
  // console.log(sum);
  return sum
},0);
